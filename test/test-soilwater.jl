...
@testset "soilwater standalone + calibration helpers" begin
  ...
  df = simulate_soilwater(forcing, dates; ps=model, state)
  @test nrow(df) == ntime
  ...
  df_pred = predict_soilwater(theta, model, forcing, dates; paths)
  ...
  gof = goodness_soilwater(theta, model, forcing, dates; paths, SM_obs, depths_SM)
