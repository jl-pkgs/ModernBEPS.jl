// _Check_Forcing.typ
// hourly 强迫数据（Forcing_Hourly_Met_sp31_v20260614.csv）的逐站逐变量值域核验 + 哨兵清洗
...
min/max/mean/p99/NA%/sentinel_n/sentinel%/outofrange%）。
...
清洗规则（物理哨兵 → NA；夜间传感器噪声 → 钳零）：

#table(
  columns: (2.4cm, 2.6cm, 4fr, 3.6cm),
  align: (left, left, left, center),
  table.header([*变量*], [*阈值*], [*策略*], [*清洗量*]),
  [Ta_canopy], [≤ -70 或 ≥ 60 ℃], [哨兵 → NA；保留 [-70, 60] 物理区间], [649 行 NA],
  [RH_canopy], [< 0 或 > 100 %], [哨兵 → NA；饱和 100% 物理合理], [466 行 NA],
  [WS_canopy], [< 0 或 > 50 m/s], [哨兵 → NA；若尔盖/达茂 357/359 m/s 极端哨兵归此], [103,900 行 NA],
  [Rs], [< 0 或 > 1400 W/m²], [负值钳零（夜间传感器噪声）；> 1400 → NA], [149,003 钳零 + 413 NA],
  [Rln_in], [< 50 或 > 500 W/m²], [哨兵 → NA；拦截盘锦/长岭/固城/锦州单位错], [70,952 行 NA],
  [Prcp], [不动], [无哨兵；小时累计 ≤ 100 mm/hr 物理合理], [—],
)
...
第二步：哨兵阈值（明显超出物理范围即判为仪器故障或单位错）：
与上同，但额外标记为 sentinel。`outofrange > 0.5%` 或 `sentinel > 0.05%` 触发报告。

#h(2em)

第三步：4 类缺陷的成因归类：
- *(a)* 整列 NA = 上游未提供数据（如临泽/三江源/若尔盖 Rs、海北高寒草甸 WS）
- *(b)* 哨兵值 = 单点仪器故障或单位错混入（如盘锦 WS -2800、固城 Ta -107）
...
不在缺陷清单内。仅当某站 Rln_in 提供了观测值但数值异常（单位错/哨兵）时才列入 §3.2。
...
#table(
  ...
  [GRA\_高寒草甸\_若尔盖], [Rs], [无辐射，外加 WS 哨兵 11.39%、Ta 哨兵 0.79%、RH 哨兵 0.91%。],
)
...
#table(
  ...
  [CRO\_春玉米\_锦州], [$-$683.6 ~ 473.4], [438.4], [单行 $-$999 哨兵混入；正常段 [100, 473]。清洗后 6.45% NA。],
  [ENF\_人工针叶林\_燕山 ×2], [0 ~ 13.4], [3.5], [98% NA；非 NA 值 < 50，疑 0 哨兵混入。],
)

== 3.3 哨兵值（单点仪器故障，清洗后归 NA）

#table(
  columns: (4.6cm, 2.2cm, 1fr, 1fr),
  align: (left, left, left, center),
  table.header([*站点*], [*变量*], [*哨兵值*], [*样本占比*]),
...
+ *风速哨兵溯源*：若尔盖 / 达茂 WS 359 m/s 量级相同，疑同一仪器问题；
